...
<div class="d-flex flex-wrap justify-content-between align-items-center mb-3 jadwal-header gap-2">
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('shift.index') }}" class="btn btn-light border btn-sm">
...
<div class="card border-0 shadow-sm rounded-3 mb-3 p-3 bg-white">
    <div class="row g-2 align-items-center">
        ...
        <div class="col-lg-5 col-md-6 d-flex align-items-center gap-2">
            <a href="{{ route('jadwal.index', ['date' => $prevWeek, 'divisi' => $filterDivisi, 'jenis' => $filterJenis]) }}" class="btn btn-light border btn-sm">
            ...
            <a href="{{ route('jadwal.index', ['date' => $nextWeek, 'divisi' => $filterDivisi, 'jenis' => $filterJenis]) }}" class="btn btn-light border btn-sm">
                <i class="bi bi-chevron-right"></i>
            </a>
            <a href="{{ route('jadwal.index', ['date' => date('Y-m-d'), 'divisi' => $filterDivisi, 'jenis' => $filterJenis]) }}" class="btn btn-outline-secondary btn-sm">
                Hari Ini
            </a>
        </div>
        ...
        <div class="col-lg-7 col-md-6">
            <form method="GET" action="{{ route('jadwal.index') }}" class="d-flex flex-wrap gap-2 justify-content-md-end">
                ...
                @if ($filterDivisi !== '' || $filterJenis !== '')
                    <a href="{{ route('jadwal.index', ['date' => $selectedDate]) }}" class="btn btn-sm btn-light border text-danger" title="Reset Filter">
...
<div class="d-flex flex-wrap gap-2 align-items-center mb-3 px-1">
    ...
    @foreach ($shifts as $s)
        @php $warna = $s->WARNA_LABEL ?? '#3b82f6'; @endphp
        <span class="badge-legend" style="background-color: {{ $warna }}15; color: {{ $warna }}; border: 1px solid {{ $warna }}40;">
            <i class="bi bi-circle-fill" style="font-size: 0.55rem;"></i> {{ $s->NAMA_SHIFT }} ({{ substr($s->JAM_MULAI, 0, 5) }} - {{ substr($s->JAM_SELESAI, 0, 5) }})
        </span>
    @endforeach
        ...
        <table class="table table-bordered table-jadwal m-0">
            <thead>
                <tr>
                    ...
                    @foreach ($weekDays as $day)
                        @php $isHoliday = isset($liburMap[$day['tanggal']]); @endphp
                        <th class="{{ $day['is_today'] ? 'today-col' : '' }} {{ $isHoliday ? 'text-danger' : '' }}">
                            <div>{{ $day['hari'] }}</div>
                            <div class="fw-normal small">{{ $day['tgl_angka'] }}</div>
                            @if ($isHoliday)
                                <span class="badge bg-danger-subtle text-danger p-0 mt-1 d-block" style="font-size: 0.65rem;" title="{{ $liburMap[$day['tanggal']] }}">
            ...
            <tbody>
                @forelse ($pegawaiList as $peg)
                    @php $avatarLetter = strtoupper(substr($peg->NM_PEGAWAI, 0, 1)); @endphp
                    <tr>
                        <td style="padding-left: 1rem;">
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <div class="text-truncate">
                                    <div class="fw-bold text-dark text-truncate" style="max-width: 140px;">{{ $peg->NM_PEGAWAI }}</div>
                                    <div class="text-muted" style="font-size: 0.72rem;">
                                        <span class="badge bg-light text-dark border me-1">{{ $peg->NAMA_DIVISI ?? 'Divisi' }}</span>
                                        {{ $peg->JENIS_PEGAWAI }}
                                    </div>
                                </div>
                            </div>
                        ...
                        @foreach ($weekDays as $day)
                            ...
                            <td class="{{ $day['is_today'] ? 'today-cell' : '' }}">
                                <div class="{{ $slotClass }}" style="{{ $slotStyle }}"
                                     ...
                                     data-nama-pegawai="{{ $peg->NM_PEGAWAI }}"
                                     data-tanggal="{{ $tgl }}"
                                     data-tgl-label="{{ $day['hari'] . ', ' . $day['tgl_angka'] }}"
                                     data-id-jadwal="{{ $idShift ?? '' }}"
                                     data-status="{{ $statusKerja }}"
                                     data-keterangan="{{ $dataJadwal->KETERANGAN ?? '' }}">
                                    ...
                                    @elseif (!empty($idShift) && isset($shiftMap[$idShift]))
                                        <div class="shift-name text-truncate">{{ $shiftMap[$idShift]->NAMA_SHIFT }}</div>
...
<div class="modal fade" id="editSlotModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form method="POST" action="{{ route('jadwal.set') }}">
            ...
            <input type="hidden" name="week_ref" value="{{ $selectedDate }}">
...
<div class="modal fade" id="bulkModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form method="POST" action="{{ route('jadwal.bulk') }}">
            @csrf
            <input type="hidden" name="week_ref" value="{{ $selectedDate }}">
                        ...
                        <select class="form-select" name="id_pegawai" required>
                            ...
                            @foreach ($pegawaiList as $p)
                                <option value="{{ $p->ID_PEGAWAI }}">
                                    {{ $p->NM_PEGAWAI }} ({{ $p->NAMA_DIVISI ?? '' }} - {{ $p->JENIS_PEGAWAI }})
                                </option>
                            @endforeach
                        </select>
                    </div>
